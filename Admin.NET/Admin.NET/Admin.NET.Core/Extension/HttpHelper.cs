...
{
    public static async Task<TResponse> GetagodaAsync<TResponse>(this string url)
    {
        ...
        {
                throw new Exception($"HTTP GET请求发生异常: {ex.Message}");
        }
    }
    public static async Task<TResponse> PostAsync<TRequest, TResponse>(string url, TRequest requestData)
    {
        ...
        {
            throw new Exception($"HTTP Post请求发生异常: {ex.Message}");
        }
    }
}
